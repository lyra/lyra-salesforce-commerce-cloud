...
function validateAndUpdateBillingPaymentInstrument(order) {
    ...
    if (!form) return;
    // Lyra Cartridge
    // No need to clean input for Lyra
...
function updatePaymentInformation(order, options) {
    ...
    if (order.billing.payment && order.billing.payment.selectedPaymentInstruments
        && order.billing.payment.selectedPaymentInstruments.length > 0) {
            if (!order.billing.payment.selectedPaymentInstruments[0].isLyra) {
                ...
                if (options && options.lyraPaymentFormUrl) {
                    $.ajax({
                        url: options.lyraPaymentFormUrl,
                        type: 'get',
                        success: function (data) {
                            $paymentSummary.empty().append(data);
                        },
                        error: function (err) {
                            console.log('Cannot retrieve lyraPaymentForm.');
                        }
                    });
                }
            }
